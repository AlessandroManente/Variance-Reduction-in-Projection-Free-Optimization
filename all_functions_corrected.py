...
def plot_all(methods, losses, times, xlimit = None, ylimit = None, name=''):
    ...
    plt.savefig('All' + name +'.png',bbox_inches='tight',dpi=300)
    plt.show()
...
def loss_gradient_vr(W, X, y, indexes):
    '''
    Computes the gradient w.r.t. the weights W. Must also pass a batch of indexes.
    Input:\\
    - X: the data matrix. M rows and N columns\\
    - W: weights. H rows (H = number of labels) and M columns (M number of 
    features)\\
...
def nuclear_projection(A, radius):
    """Projection onto the nuclear norm ball with raudius tau."""
...
def svrf(W, X, y, epochs, step_width = 2, lr=None, epsilon=None, time_to_run = None):
    ...
    for j in range(1, epochs + 1):
        ...
        for i in range(1, 50+1):
            ...
            batch_size = i+1
            list_indexes = random.sample(range(N), batch_size)
            gradient_svrf = loss_gradient_vr(W_svrf, X, y, list_indexes) - loss_gradient_vr(W_snap, X, y, list_indexes) + snapshot
...
#   W_storc = -tau * np.matmul(u, v) 
    ...
    for i in range(1, epochs + 1):
        ...
        for j in range(1, 50+1):
            ...
            eta = 1/iteration
            m_tk = 100 
            list_indexes = random.sample(range(N), m_tk)
            
            z = Y + gamma * (x - Y)
            a = loss_gradient_vr(z, X, y, list_indexes)
            b = loss_gradient_vr(W_snap, X, y, list_indexes)
...
def svrg(W, X, y, lr, epochs, maxit, step_width, epsilon=None, time_to_run = None, s = 1, batch_size = 1):
    ...
    for j in range(1, epochs + 1):
        ...
        for i in range(1, 50+1):
            list_indexes = random.sample(range(N), batch_size)
            gradient_svrg = loss_gradient_vr(W_svrg, X, y, list_indexes)  - loss_gradient_vr(W_snap, X, y, list_indexes) + snapshot
